...
export default class AdminMiddleware {
  public async handle({ auth, response, request }: HttpContext, next: () => Promise<void>) {
   ...
   const tontineId = request.param('tontine_id') || request.param('id') || request.body().tontine_id
   ...
   // ici on récupère correctement l'id de la tontine depuis le body de la requête
    ...
    if (!user) {
       ...
       if (!tontineId) {
      return response.badRequest({ message: 'Identifiant de la tontine manquant.' })
    }
